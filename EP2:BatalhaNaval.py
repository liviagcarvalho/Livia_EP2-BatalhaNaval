...
def posicao_suporta(mapa_computador, blocos, linha, coluna, orientacao):
    if linha < 0 or coluna < 0 or linha >= len(mapa_computador) or coluna >= len(mapa_computador):
        return False
    if orientacao not in ['v', 'h']:
        return False
    if orientacao == 'v':
        if linha + blocos > len(mapa_computador):
            return False
        for i in range(linha, linha + blocos):
            if mapa_computador[i][coluna] != ' ':
                return False
    elif coluna + blocos > len(mapa_computador):
        return False
    else:
        for j in range(coluna, coluna + blocos):
            if mapa_computador[linha][j] != ' ':
                return False
    return True

    # Alocando navios para o computador
def aloca_navios(mapa_computador, blocos):
    n = len(mapa_computador)
    for navio in blocos:
        ...
        while not alocando_navio:
            ...
            if posicao_suporta(mapa_computador, navio, linha, coluna, orientacao):
                if orientacao == 'v':
                    for i in range(linha, linha + navio):
                        mapa_computador[i][coluna] = '🛳'
                else:
                    for j in range(coluna, coluna + navio):
                        mapa_computador[linha][j] = '🛳'
                alocando_navio = True
...
def batalha_naval():
    ...
    while jogar_novamente == 's':
        ...
        print(" ")
        print(" ")
        print(CORES['magenta'] + "                            ###" + CORES['reset'])
        print(CORES['magenta'] + "                     #" + CORES['reset'])
        print(CORES['magenta'] + "                     #############" + CORES['reset'])
        print(CORES['magenta'] + "                     ###   #  # ####" + CORES['reset'])
        print(CORES['magenta'] + "                     ###   ####   ##" + CORES['reset'])
